...
class Twitter {
  ...
  public function user_timeline($id, $params = array(), $use_auth = FALSE) {
    ...
    return $this->get_statuses('statuses/user_timeline', $params, 1);
  }
   ...
   * @see http://apiwiki.twitter.com/Twitter-REST-API-Method%3A-statuses-user_timeline
   */
  public function home_timeline($id, $params = array(), $use_auth = TRUE) {
    if (is_numeric($id)) {
      $params['user_id'] = $id;
    }
    else {
    ...
    return $this->get_statuses('statuses/home_timeline', $params, TRUE);
  }
